...
const TeamEditPage = () => {
  ...
  const fetchTeam = async () => {
    try {
      ...
      setFormData({
        ...
        contestId: team.contest?._id || team.contestId || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Note: API might not support full team updates, so we'll try the update endpoint
      await apiService.updateAdminTeam(id, formData);
      toast.success('Cập nhật đội thi thành công');
      navigate(`/admin/teams/${id}`);
    } catch (error) {
      console.error('Error updating team:', error);
      toast.error('Có lỗi khi cập nhật đội thi');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Chỉnh sửa đội thi"
        description={`Cập nhật thông tin đội thi #${id}`}
        actions={pageActions}
      />
